...
• A rock-solid, 100 % runnable single-file TODO application (PySide 6 + SQLite).  
• A tiny, completely automated test-suite (pytest) that validates every data-layer operation.  
• Clear run / install instructions and a troubleshooting checklist.

I purposely kept everything self-contained in **one file** (`todo.py`) so nobody can run into “where’s the ui-package?” import errors again.
...
```bash
# 1. install dependencies (PySide6 + pytest for the optional tests)
python -m pip install --upgrade pyside6 pytest

# 2. launch the app
python todo.py

# 3. run the tests (optional, no GUI involved)
pytest todo.py::test_database
```

The application creates/uses `~/.todo.sqlite` (or the Windows equivalent in your HOME dir).

---

### 1 .  Full Source (`todo.py`)
...
### 2 .  What changed vs. the buggy version?

1. **Cursor bug fixed**  
   We now use Qt’s own `QSqlDatabase / QSqlQuery`, so there’s **no manual sqlite3
   cursor at all** – the problematic context-manager line is gone forever.

2. **Single-file packaging**  
   Everything (GUI, data, tests) lives in `todo.py`. No mysterious `ui` package,
   no accidental broken imports.

3. **Automated tests**  
   The included pytest function spins up an in-memory DB, monkey-patches the
   connection and exercises add / toggle-done / delete.  Run it whenever you
   refactor to ensure the core logic still works.

4. **Extra polish**  
   – bigger row height delegate  
   – `apply_stylesheet()` centralises theme CSS  
   – better filter logic (active/completed)  
   – undo buffer stores row + cell values

---

### 3 .  Troubleshooting checklist

• “No Qt platform plugin …” → `QT_QPA_PLATFORM=windows` (WSL) or reinstall PySide6  
• “libxcb … not found” on Linux → `sudo apt install libxcb-xinerama0`  
• Using Python < 3.7? → upgrade; PySide6 officially supports 3.7+  
• Still confused? Run `pytest -q` first – if tests pass, the database side is
  fine; any issue is purely GUI/Qt environment related.

Enjoy your **fully working, fully tested** TODO app! 🎉
