Love it. Let’s tailor this specifically for Claude Code so you can drop it into claude.ai/code and get consistently great, on-style summaries.

Plan and checklist
- Goal: Provide a Claude Code–optimized prompt template that produces a single, polished CLAUDE.md mirroring your sample structure and tone.
- Claude-specific behaviors:
  - Use static repo analysis via Claude Code file tree and search.
  - No speculation; path-cite every non-obvious claim.
  - Omit sections not supported by evidence.
  - Include only small, real code snippets with path headers.
- Output controls:
  - Single markdown file, no extra commentary.
  - UI-only sections appear conditionally.
  - Monorepo-aware with concise per-package notes.

Self-check before you run this
- [ ] DOC_NAME defaults to CLAUDE.md
- [ ] AGENT_NAME locked to Claude Code
- [ ] Accuracy and safety rules in place
- [ ] Sections match the sample’s layout and tone
- [ ] Clear final self-check inside the template

Claude Code–optimized prompt template (paste into claude.ai/code)
```text
...
If you’d like, I can also add a compact “starter message” you can save as a Claude Code preset with DOC_NAME and AGENT_NAME pre-filled and ready to run against any repo.
